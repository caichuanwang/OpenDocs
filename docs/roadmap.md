...
状态：计划已确认；无向下兼容负担（0.1.0 无下游用户）；尚未开始实现。
...
- 新增 Excel/XLSX 作为一个需求驱动的新格式，不承接完整 M4 范围。
- 在实现前固定 XLSX 提取、合并单元格、非文本降级和资源限制行为。
- 解决公开 `max_pages` 语义，不伪造 DOCX 物理页。
...
- 基于实际需求增加更多格式，如 Excel、HTML 或 email。
